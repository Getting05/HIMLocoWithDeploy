...
// Default power-on joint angles [rad] (initial pose at encoder calibration)
constexpr std::array<float, NUM_JOINTS> DEFAULT_DOF_POS = {
    -0.35f, 0.99f, -2.57f, // FR: hip, thigh, calf
    0.35f,  0.99f, -2.57f, // FL: hip, thigh, calf
    -0.35f, 0.99f, -2.57f, // RR: hip, thigh, calf
    0.35f,  0.99f, -2.57f, // RL: hip, thigh, calf
};

// Standing target pose [rad] (StandUp state interpolation target)
constexpr std::array<float, NUM_JOINTS> STANDUP_TARGET_POS = {
...
// Policy reference pose [rad] (MUST match mybot_config.py default_joint_angles)
// Used for observation: (dof_pos - POLICY_DOF_POS) * scale
// Used for action→target: target = POLICY_DOF_POS + action * action_scale
constexpr std::array<float, NUM_JOINTS> POLICY_DOF_POS = {
    -0.1f, 0.8f, -1.5f, // FR: hip, thigh, calf
    0.1f,  0.8f, -1.5f, // FL: hip, thigh, calf
    -0.1f, 1.0f, -1.5f, // RR: hip, thigh, calf
    0.1f,  1.0f, -1.5f, // RL: hip, thigh, calf
...
constexpr std::array<float, NUM_JOINTS> JOINT_POS_LOWER = {
    -0.48f, -1.44f, -2.50f, // FR
    -0.48f, -1.44f, -2.50f, // FL
    -0.48f, -1.44f, -2.50f, // RR
    -0.48f, -1.44f, -2.50f, // RL
};

constexpr std::array<float, NUM_JOINTS> JOINT_POS_UPPER = {
    0.48f, 1.44f, -1.10f, // FR
    0.48f, 1.44f, -1.10f, // FL
    0.48f, 1.44f, -1.10f, // RR
    0.48f, 1.44f, -1.10f, // RL
};
...
// ========================== State Machine ==========================
constexpr float STANDUP_DURATION =
    2.0f; // seconds to interpolate to standing pose
